...
Material *SceneParser::parseMaterial() {
    ...
    Material::ReflectType refl_type = Material::kDiffuse;
    ...
    while (true) {
        ...
        if (strcmp(token, "ambientColor") == 0) {
            ...
            getToken(filename);
        } else if (strcmp(token, "reflectType") == 0) {
            refl_type = Material::ReflectType(readInt());
    ...
    auto *answer = new Material(filename, ambientColor, diffuseColor, specularColor, refl_type, refractive_index);
    return answer;
}
...
void SceneParser::parseLights() {
    ...
    for (int i = 0; i < group->size(); ++i) {
        Sphere *p;
        if ((p = dynamic_cast<Sphere *>((*group)[i])) != nullptr && 
             p->material()->ambientColor() != Vector3f::ZERO) {
            ++num_lights;
        }
    }
